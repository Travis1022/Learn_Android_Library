package com.sunnybear.library.recycler.expandable.adapter;
...
import com.sunnybear.library.recycler.expandable.model.ExpandableListItem;
import com.sunnybear.library.recycler.expandable.viewholder.AbstractAdapterItem;
import com.sunnybear.library.recycler.expandable.viewholder.AbstractExpandableAdapterItem;
import com.sunnybear.library.recycler.expandable.viewholder.AdapterItemUtil;
import com.sunnybear.library.recycler.expandable.viewholder.BaseAdapterItem;
 ...
 */
public abstract class ExpandableAdapter<T extends Serializable> extends RecyclerView.Adapter implements AbstractExpandableAdapterItem.ParentListItemExpandCollapseListener {
    ...
    protected ExpandableAdapter(List data) {
        if (data == null) return;
        this.mDataList = data;
        checkDefaultExpand();
    ...
    private void checkDefaultExpand() {
        ArrayMap<T, List<T>> childArrayMap = new ArrayMap<>();
        ...
        while (iterator.hasNext()) {
            ...
            if (next instanceof ExpandableListItem) {
                ...
                if (expandableListItem.isExpanded()) {
                    List<T> childItemList = (List<T>) expandableListItem.getChildItemList();
        ...
        for (int i = 0; i < size; i++) {
            ...
            List<T> objects = childArrayMap.valueAt(i);
     ...
     * @param o        item
     */
    public void addItem(int position, T o) {
     ...
     */
    public void addRangeItem(int position, List<T> objects) {
     ...
     */
    public void modifyItem(int position, T newObj) {
        if (isDataListNotEmpty() && position < mDataList.size() && position >= 0) {
    ...
    private void notifyItemExpandedOrCollapsed(int parentIndex, boolean isExpand) {
        if (mRecyclerViewList != null && !mRecyclerViewList.isEmpty()) {
            ...
            try {
                AbstractAdapterItem<Serializable> item = viewHolderForAdapterPosition.getItem();
...
//    }
    ...
    @NonNull
    public abstract AbstractAdapterItem<Serializable> getItemView(Object type);
